...
A subset of the 2010 MeSH thesaurus from the NLM and a list of non-MeSH (free text) disease condition terms provided by data submitters that appeared in 5 or more interventional studies in the analysis data set were reviewed and annotated by clinical specialists at Duke University Medical Center. Terms were annotated according to their relevance to a given specialty (Y = relevant, N = not relevant). Cardiovascular(N=3437), oncology(N=8992), and mental health(N=3695)studies were identified by searching for relevant MeSH and free text terms in the submitted conditions field, as well the condition MeSH terms generated for each study by the NLM.Because MeSH terms can exist with multiple MeSH IDs in the MeSH thesaurusand relevance to a clinical specialty may depend on the context, only MeSH terms that were consistently coded as relevant (Y) across all MeSH IDs met the criteria for inclusion in this analysis. A single study may have multiple MeSH and/or condition terms, and studies were included if they had one or more terms relevant to the clinical specialty of interest. A study could be included in more than one specialty. The results of these specialty classifications based on searches for relevant terms were validated by comparison with classifications based on manual review. (asneem A, Aberle L, Ananth H, et al. The database for Aggregate Analysis of ClinicalTrials.gov (AACT) and subsequent regrouping by clinical specialty. PLoS One. 2012;7(3):e33677.)",
      }
    end
