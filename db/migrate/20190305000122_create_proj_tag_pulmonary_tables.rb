class CreateProjTagPulmonaryTables < ActiveRecord::Migration[5.2]
  ...
  def up
    ...
    #  the view.
    #execute <<-SQL
    #    CREATE OR REPLACE FUNCTION proj_tag_pulmonary.create_views() RETURNS void AS
    #    $BODY$
    #    BEGIN
    #      EXECUTE 'CREATE OR REPLACE VIEW proj_tag_pulmonary.tagged_studies AS ' ||
    ...
    #             ' and bc.downcase_mesh_term IN (select term from proj_tag_pulmonary.tagged_terms) '
    #          ' union ' ||
    #          '   select distinct s.nct_id ' ||
    ...
    #          '  and k.downcase_name IN (select term from proj_tag_pulmonary.tagged_terms)  ' ||
    #          ' union  ' ||
    #          '   select distinct s.nct_id  ' ||
    ...
    #          '   and c.downcase_name IN (select term from proj_tag_pulmonary.tagged_terms)  ' ||
    #          '   ) ';
#
##          RETURN;
#        END;
