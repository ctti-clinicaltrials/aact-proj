...
namespace :db do
  ...
  task create: [:environment] do
    ...
    con.execute("alter role read_only in database #{ENV['AACT_PUBLIC_DATABASE_NAME']} set search_path = public, mesh_archive, #{Admin::Project.schema_name_list};")
    con.execute("alter role read_only in database #{ENV['AACT_PUBLIC_DATABASE_NAME']}_alt set search_path = public, mesh_archive, #{Admin::Project.schema_name_list};")
    con.reset!
